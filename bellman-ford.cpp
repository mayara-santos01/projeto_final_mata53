...
unordered_map<string, int> bellman_ford(vector<Vertice>& graph, const string& raiz) {
  // Passo 1: Inicializar distâncias
  unordered_map<string, int> distancias;
  for (const auto& vertice : graph) {
    distancias[vertice.label] = numeric_limits<int>::max();
  }
  distancias[raiz] = 0;

  // Passo 2: Relaxar as arestas repetidamente 
  for (size_t i = 0; i < graph.size() - 1; ++i) {
    for (const auto& vertice : graph) {
      for (const auto& aresta : vertice.arestas) {
        const string& u = vertice.label;
        const string& v = aresta.first;
  ...
  for (const auto& vertice : graph) {
    for (const auto& aresta : vertice.arestas) {
      ...
      int peso = aresta.second;
      if (distancias[u] != numeric_limits<int>::max() && distancias[u] + peso < distancias[v]) {
        throw runtime_error("O grafo contem um ciclo de peso negativo");
      }
    }
  }
...
int main() {
  ...
  try {
    auto distancias = bellman_ford(graph, "ic1_1");
    for (const auto& pair : distancias) {
      cout << "Menor distancia de ic1_1 para " << pair.first << " eh " << pair.second << endl;
    }
