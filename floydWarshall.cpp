...
void floydWarshall(vector<vector<int>>& dist, const vector<Vertice>& graph) {
    int n = dist.size();
    unordered_map<string, int> vertex_indices;
    ...
    for (const auto& vertex : graph) {
        if (vertex.label[0] == 'i') {
            inner_core_indices.push_back(vertex_indices[vertex.label]);
        }
    }
...
int main() {
    ...
    floydWarshall(distance_matrix, graph);
