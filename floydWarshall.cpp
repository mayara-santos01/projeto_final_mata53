...
vector<vector<int>> create_distance_matrix(const vector<Vertice>& graph) {
    ...
    for (int i = 0; i < num_vertices; i++) {
        ...
        for (const auto& edge : graph[i].arestas) {
            int j = vertex_indices[edge.first]; 
...
void floydWarshall(vector<vector<int>>& dist) {
    ...
    cout << "Matriz de distância dos caminhos mais curtos:" << endl;
    imprimirMatriz(dist);
}
...
int main() {
    ...
    vector<vector<int>> distance_matrix = create_distance_matrix(graph);
    ...
    int index_ic1_1 = vertex_indices["ic1_1"];
    int index_ac57 = vertex_indices["ac57"];

    cout << "\n A menor distancia de ic1_1 para ac57 é: ";
    if (distance_matrix[index_ic1_1][index_ac57] == INT_MAX) {
        cout << "INF" << endl;
    } else {
        cout << distance_matrix[index_ic1_1][index_ac57] << endl;
    }
